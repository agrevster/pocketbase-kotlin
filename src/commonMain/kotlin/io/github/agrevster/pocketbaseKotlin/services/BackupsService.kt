...
public class BackupsService(client: PocketbaseClient) : BaseService(client) {
     ...
     */
    @Untested("I don't want to bother embedding a whole zip file into the project. Reading files in a multi platform way is hard too. You're just going to have to trust me...")
    public suspend fun upload(file: FileUpload): Unit {
        val response = client.httpClient.post {
            url {
                path("api", "backups", "upload")
            }
            setBody(MultiPartFormDataContent(formData {
                append("file", file.file!!, headers = headersOf(HttpHeaders.ContentDisposition, "filename=${file.fileName.escapeIfNeeded()}"))
            }))
        }
        PocketbaseException.handle(response)
    }
